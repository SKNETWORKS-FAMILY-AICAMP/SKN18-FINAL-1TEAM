...
export default function LandDetail({ landId }: LandDetailProps) {
  ...
  const tempExplain: Record<TempId, { title: string; subtitle: string; body: string[] }> = {
    convenience: {
      title: '생활편의',
      subtitle: '일상 편의시설 접근성',
      body: [
        '편의점, 마트, 세탁소 등 일상생활에 필수적인 시설까지의 거리를 평가합니다.',
        '가까울수록 높은 점수를 부여하며, 도보 생활권 내 편의성을 측정합니다.',
        '병원, 약국 등 의료시설 접근성도 함께 반영합니다.',
      ],
    },
    traffic: {
      title: '교통',
      subtitle: '대중교통 접근성 + 출퇴근 편의성',
      body: [
        '가장 가까운 지하철역의 이용량과 노선 수를 기반으로 교통 편의성을 평가합니다.',
        '강남, 여의도 등 주요 업무지구까지의 거리를 반영해 출퇴근 편의성을 측정합니다.',
        '주변 버스정류장 수를 고려해 대중교통 접근성을 종합적으로 점수화합니다.',
      ],
    },
    culture: {
      title: '문화',
      subtitle: '문화·여가 시설 접근성',
      body: [
        '영화관, 미술관, 공연장 등 문화시설까지의 거리를 평가합니다.',
        '도서관, 박물관 등 교육·문화 인프라 접근성을 반영합니다.',
        '공원, 녹지공간 접근성도 함께 고려해 여가 환경을 종합 평가합니다.',
      ],
    },
    safety: {
      title: '안전',
    ...
    pet: {
      ...
      subtitle: '반려동물 친화 환경',
      body: [
        '반려동물 놀이터, 동물병원, 펫샵 등 필수 시설 접근성을 평가합니다.',
        '산책하기 좋은 공원(1,500㎡ 이상)까지의 거리를 반영합니다.',
        '가까울수록 높은 점수를 부여해 반려인 생활 편의성을 측정합니다.',
      ],
    },
  };
                ...
                >
                  {activeTempItem && activeTemp && (
                    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                      ...
                      📌 참고: 온도는 전체 매물 분포 대비 상대적 위치를 보여주며, 36.5°C는 평균 기준선입니다.<br />
                      <br />
                      온도가 높을수록 해당 카테고리에서 우수한 환경임을 의미합니다.
                      </div>
                    </div>
                  )}
                </div>
